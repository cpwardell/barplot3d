...
## Set the window size and save the image
par3d(windowRect=c(0,50,400,450))
rgl.snapshot("simple.png")
rgl.close()
```
...
rgl.snapshot("pretty.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./pretty.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,400,450))
rgl.snapshot("gridexample1.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./gridexample1.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,400,450))
rgl.snapshot("gridexample2.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./gridexample2.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,400,450))
rgl.snapshot("zerogapped.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./zerogapped.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,400,450))
rgl.snapshot("scalexy_skinny.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./scalexy_skinny.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,600,450))
rgl.snapshot("scalexy_fatty.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./scalexy_fatty.png')
```
...
* "phi" rotates the viewpoint in hte vertical plane (imagine going below and above the plot) and can be set between -90 to 90 (-90 is directly below, 90 directly above).

The plot size is determined by the par3d() function. Again, you will have to experiment to find the right settings that make your plot and labels 
fully visible.  The 4 arguments are the position (in pixels) of the left, top, right and bottom edges of the rgl viewing window. e.g. par3d(windowRect=c(0,50,600,450)) leads to plot that is 600 pixels wide and 400 pixels tall.

rgl.snapsnot("filename.png") is used to save the current RGL view. Note that _only_ png format is currently supported.
...
2. "sidecolors" specifies the colors of the sides of each bar (default is grey)
3. "linecolors" specifies the colors of the edges of each bar (default is black)

Transparency of the sides of the bars can be set using the "alpha" parameter. 0 is completely transparent (i.e. a "wireframe" effect) and 1 (the default) is completely opaque. This can be useful when taller bars hide shorter bars, but if used you may want a neutral color like grey so that the topcolors are not altered by the partically transparent colored bar in front of them.
...
## Set the window size and save the image
par3d(windowRect=c(0,50,600,850))
rgl.snapshot("manylabels.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./manylabels.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,600,850))
rgl.snapshot("minimalism.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./minimalism.png')
```
...
in sequencing data. I _think_ their first use was in Figure 1 of [Exome and whole genome sequencing of esophageal adenocarcinoma identifies recurrent driver events and mutational complexity by Dulak et al, 2013](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3678719/pdf/nihms474888.pdf).
...
## Set the window size and save the image
par3d(windowRect=c(0,50,600,450))
rgl.snapshot("sig2sanger.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./sig2sanger.png')
```
...
## Set the window size and save the image
par3d(windowRect=c(0,50,600,450))
rgl.snapshot("sig8broad.png")
rgl.close()
```

```{r echo=FALSE}
knitr::include_graphics('./sig8broad.png')
```
